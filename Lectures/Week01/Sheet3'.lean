...
example : f 0 3 := by
  rewrite [f]
  sorry -- we can use the rewrite tactic, but we cannot close the goal
...
example (a b : ℕ) (h1 : a = b) : a = b := by
  ...
  -- we can also tell Lean the contradicting hypotheses directly using `exact h2 h1` (first the negation)
...
example (x y : ℕ) : f 0 x ∧ f 0 y → x = y := by
  rewrite [f] -- replace one occurrence of f
  rewrite [f] -- replace second occurrence of f
  intro ⟨hx, hy⟩
  rewrite [← hx]
  rewrite [← hy]
  rfl
